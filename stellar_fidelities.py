...
def Optim_function_Fock(optim_parameters,level_n,state):
    ...
    non_zeros=np.nonzero(state)
    overlap=0.0
    if len(non_zeros[0])==1:
        for p in range(level_n+1):        
           overlap+= np.abs(Fock_coefficient(p, non_zeros[0][0], optim_parameters))**2
    else:
        for p in range(level_n+1):    
           for i in non_zeros[0]:    
            overlap+= np.abs(state[i]*Fock_coefficient(p, i, optim_parameters))**2
...
def find_optimal_fidelity(n_truncate, level_n,state):
